...
def run_teacher_forcing(model, tokenizer, samples, device):
    ...
    for si, s in enumerate(samples):
        ...
        all_sample_metrics.append({
            ...
            "recall_n2": accum["recall_next2"],
            "exact_n1": accum["exact_next1"],
            "exact_n2": accum["exact_next2"],
        ...
        for pos in range(seq_len):
            ...
            for li in moe_layer_indices:
                ...
                if li in hidden_records and pos < hidden_records[li].shape[1]:
                    ...
                    ln1, ln2 = predictor(h, li)
                    pred_n1 = torch.topk(ln1[0, 0], k=top_k).indices.cpu().tolist()
                    pred_n2 = torch.topk(ln2[0, 0], k=top_k).indices.cpu().tolist()
                    token_preds[li] = {"next1": pred_n1, "next2": pred_n2}
...
def simulate_expert_cache(all_token_data, moe_layer_indices, n_experts, top_k, cache_sizes):
    ...
    for cs in cache_sizes:
        ...
        for td in all_token_data:
            ...
            # Prefetch: use predictions from layer i to prefetch for layer i+1 and i+2
            for anchor_li in sorted(preds.keys()):
                t1 = anchor_li + 1
                if t1 in prefetch_caches:
                    prefetch_caches[t1].prefetch(preds[anchor_li]["next1"])
                t2 = anchor_li + 2
                if t2 in prefetch_caches:
                    prefetch_caches[t2].prefetch(preds[anchor_li]["next2"])
...
def main():
    ...
    avg_r2 = sum(m["recall_n2"] for m in sample_metrics) / len(sample_metrics)
    avg_e1 = sum(m["exact_n1"] for m in sample_metrics) / len(sample_metrics)
    avg_e2 = sum(m["exact_n2"] for m in sample_metrics) / len(sample_metrics)
    print(f"\n  Average recall_n1={avg_r1:.4f}, recall_n2={avg_r2:.4f}")
    print(f"  Average exact_n1={avg_e1:.4f}, exact_n2={avg_e2:.4f}")
    print(f"  Training metric (step 1151): recall_n1=0.9104, recall_n2=0.8851")
