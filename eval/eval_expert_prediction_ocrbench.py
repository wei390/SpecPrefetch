...
class DecodeRecallAccumulator:
    ...
    def add(self, recall_n1: float, exact_n1: float, has_n1: bool = True):
        self.n_tokens += 1
        if has_n1:
...
def generate_and_measure_recall(model, input_ids: torch.Tensor, pixel_values: torch.Tensor | None,
    ...
    for step in range(max_new_tokens - 1):
        ...
        accum.add(avg_recall_n1, avg_exact_n1, has_n1)
...
def print_results(all_metrics, per_category, args):
    ...
    print(f"\n  Overall Results:")
    print(f"  {'Metric':<25s} {'Next-1':<12s}")
    print(f"  {'-'*37}")
    print(f"  {'Top-K Recall':<25s} {avg_recall_n1:<12.4f}")
    print(f"  {'Exact Match':<25s} {avg_exact_n1:<12.4f}")

    if per_category:
        print(f"\n  Per-Category Results (Top-K Recall):")
        print(f"  {'Category':<30s} {'Count':<8s} {'Recall-N1':<12s} {'Exact-N1':<12s}")
        print(f"  {'-'*62}")
        for cat in sorted(per_category.keys()):
            ms = per_category[cat]
            r1 = np.mean([m["recall_n1"] for m in ms])
            e1 = np.mean([m["exact_n1"] for m in ms])
            print(f"  {cat:<30s} {len(ms):<8d} {r1:<12.4f} {e1:<12.4f}")
