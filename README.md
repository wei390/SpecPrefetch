...
predicts the **Top-K expert set** that the *teacher router* will select one
layer later (`l+1`). The predictions are used at inference time to **prefetch
experts** before the router gets there, hiding cold-load latency on GPUs with
limited expert cache.
...
| `FUTURE_EXPERT_PREDICTOR_HORIZONS`        | `1`       | Future layer to predict (`l+1`). Only horizon=1 is supported.                            |
| `FUTURE_EXPERT_COEF_NEXT1`                | `2.0`     | KL-loss weight for the next-1 horizon.                                                   |
...
top_ks` measurements per sample, so 50 samples per dataset typically
already lands within ±0.01 of the full-set numbers.
...
Quoting the headline numbers (n_samples / `next1 recall@3`):

| Dataset           | n     | next1 r@3 | next1 r@6 | next1 r@8 | next1 exact@6 |
| ----------------- | ----- | --------- | --------- | --------- | ------------- |
...
- **Single-horizon build**: this code only predicts `l+1`. Old checkpoints
  trained with horizons `[1, 2]` can still be loaded — the `next2_head`
  weights are silently ignored as `unexpected keys` since `strict=False` is
  used in `load_pretrained_weights`. Re-save the checkpoint with the
  current architecture if you want a clean state dict.
