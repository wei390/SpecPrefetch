...
class FutureExpertPredictorMetricsCallback(TrainerCallback):
    ...
    def on_train_begin(self, args, state, control, **kwargs):
        ...
        try:
            ...
            wandb.define_metric("train/loss_future_expert_next2", step_metric="train/global_step", summary="min")
            wandb.define_metric("train/loss_next1_kl", step_metric="train/global_step", summary="min")
            wandb.define_metric("train/loss_next2_kl", step_metric="train/global_step", summary="min")
            wandb.define_metric("train/recall_next1", step_metric="train/global_step", summary="max")
            wandb.define_metric("train/recall_next2", step_metric="train/global_step", summary="max")
            wandb.define_metric("train/exact_next1", step_metric="train/global_step", summary="max")
            wandb.define_metric("train/exact_next2", step_metric="train/global_step", summary="max")
            ...
            wandb.define_metric("train/fusion_alpha_h2_mean", step_metric="train/global_step")
    ...
    def on_log(self, args, state, control, logs=None, model=None, **kwargs):
        ...
        try:
            ...
            metrics = {
                ...
                "train/loss_future_expert_next2": self._safe_float(accum.get("loss_future_expert_next2", 0.0)) / n,
                "train/loss_next1_kl": self._safe_float(accum.get("loss_next1_kl", 0.0)) / n,
                "train/loss_next2_kl": self._safe_float(accum.get("loss_next2_kl", 0.0)) / n,
                "train/recall_next1": self._clamp_01(self._safe_float(accum.get("recall_next1", 0.0)) / n),
                "train/recall_next2": self._clamp_01(self._safe_float(accum.get("recall_next2", 0.0)) / n),
                "train/exact_next1": self._clamp_01(self._safe_float(accum.get("exact_next1", 0.0)) / n),
                "train/exact_next2": self._clamp_01(self._safe_float(accum.get("exact_next2", 0.0)) / n),
                ...
                "train/fusion_alpha_h2_mean": self._safe_float(accum.get("fusion_alpha_h2_mean", 0.0)) / n,
            }
            ...
            print(
                ...
                f"next2={metrics['train/loss_future_expert_next2']:.6f}/"
                f"r{metrics['train/recall_next2']:.6f}/e{metrics['train/exact_next2']:.6f} "
                f"next1_kl={metrics['train/loss_next1_kl']:.6f} "
                f"next2_kl={metrics['train/loss_next2_kl']:.6f} "
                f"alpha_h1={metrics['train/fusion_alpha_h1_mean']:.4f} "
                f"alpha_h2={metrics['train/fusion_alpha_h2_mean']:.4f}",
                flush=True,
            )
